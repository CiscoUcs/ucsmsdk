...
class LsAgentPolicy(ManagedObject):
    ...
    mo_meta = MoMeta("LsAgentPolicy", "lsAgentPolicy", "agent-policy-[name]", VersionMeta.Version101e, "InputOutput", 0x1ffL, [], ["admin", "ls-config-policy", "ls-server-policy"], [u'orgOrg'], [], [None])

    prop_meta = {
        "capability": MoPropertyMeta("capability", "capability", "string", VersionMeta.Version101e, MoPropertyMeta.READ_WRITE, 0x1L, None, None, None, ["host-name-config", "l2-if-config", "l3-if-config", "states", "stats"], []), 
        ...
        "mode": MoPropertyMeta("mode", "mode", "string", VersionMeta.Version101e, MoPropertyMeta.READ_WRITE, 0x10L, None, None, None, ["full", "no-agent", "read-only"], []), 
