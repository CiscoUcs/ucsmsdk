...
class MgmtDbState(ManagedObject):
    ...
    mo_meta = MoMeta("MgmtDbState", "mgmtDbState", "mgmt-db-state", None, "InputOutput", 0x3f, [], ["read-only"], [u'networkElement'], [u'faultInst'], [None])
    ...
    prop_meta = {
        ...
        "corrupted_count": MoPropertyMeta("corrupted_count", "corruptedCount", "ulong", None, MoPropertyMeta.READ_WRITE, 0x4, None, None, None, [], []), 
        "dn": MoPropertyMeta("dn", "dn", "string", None, MoPropertyMeta.READ_ONLY, 0x8, 0, 256, None, [], []), 
        "last_occurrence_time": MoPropertyMeta("last_occurrence_time", "lastOccurrenceTime", "string", None, MoPropertyMeta.READ_ONLY, None, None, None, r"""([0-9]){4}-([0-9]){2}-([0-9]){2}T([0-9]){2}:([0-9]){2}:([0-9]){2}((\.([0-9]){3})){0,1}""", [], []), 
