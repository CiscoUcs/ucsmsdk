...
class MoKvInvHolder(ManagedObject):
    ...
    mo_meta = MoMeta("MoKvInvHolder", "moKvInvHolder", "inv-kv-[endpoint]", None, "InputOutput", 0x3f, [], ["read-only"], [u'computeBlade', u'computeRackUnit', u'computeServerUnit'], [u'moInvKv'], [None])
    ...
    prop_meta = {
        ...
        "endpoint": MoPropertyMeta("endpoint", "endpoint", "string", None, MoPropertyMeta.NAMING, 0x8, 1, 32, None, [], []), 
        "last_update": MoPropertyMeta("last_update", "lastUpdate", "string", None, MoPropertyMeta.READ_ONLY, None, None, None, r"""([0-9]){4}-([0-9]){2}-([0-9]){2}T([0-9]){2}:([0-9]){2}:([0-9]){2}((\.([0-9]){3})){0,1}""", ["never"], []), 
